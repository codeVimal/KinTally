...
export default function Statistics() {
  ...
  /* ---- filter transactions ---- */
  const filtered = DUMMY_TX.filter((t) => {
  ...
  const chartData = Object.entries(totals).map(([cat, amt]) => ({
    ...
    amount: amt,
    color:  CATEGORY_META[cat]?.color || '#888',   // fallback color avoids crash
    legendFontColor: '#000',
    legendFontSize: 14,
  }));
...
const styles = StyleSheet.create({
  ...
  row: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 0.5, borderBottomColor: '#ddd' },
